...
new_data <- 
  expand_grid(
    ...
    period_static = seq(ymd("2017-01-01"), ymd("2021-02-28"), "month")) %>%
  wrangle_new_data()
...
annual_2020 <- mutate(estimations_gear, est = "accounting for fishing gear") %>%
  ...
  filter(period_static >= ymd("2020-01-01"), period_static <= ymd("2020-12-01")) %>% 
  group_by(est, .draw) %>%
  summarise(across(where(is.numeric), sum))

annual_2019 <- mutate(estimations_gear, est = "accounting for fishing gear") %>%
  bind_rows(mutate(estimations_no_gear, est = "not accounting for fishing gear")) %>%
  filter(period_static >= ymd("2019-01-01"), period_static <= ymd("2019-12-01")) %>% 
  group_by(est, .draw) %>%
  summarise(across(where(is.numeric), sum))
